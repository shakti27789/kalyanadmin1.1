...
<div class="game-heading rounded-right-2">
					<span class="card-header-title">1 DAY DRAGON TIGER  Rules  &nbsp; 
...
<div class="score-panel">
					<div class="lucky7-screen">
						<iframe _ngcontent-hnv-c80="" width="100%" height="420" style="border: 0px;" src="https://shroute.casinovid.in/diamondvideo/sh.php?id=3057"></iframe>
						
	              <!--     <iframe _ngcontent-hnv-c80="" width="100%" height="420" style="border: 0px;" src="https://cricexch.live/video/dragontigeroneday"></iframe>
				-->	
					  ...
					  <tbody>
						...
						<!-- for first row start-->
						 ...
						 <tr data-title="" class="bet-info">
							<td class="box-5"><b>{{jdata.t2.[0].nat}}<span id="sid1"></span></b></td>
						 ...
						 <tr data-title="" class="bet-info" id="bet-info">
							<td class="box-5"><b>{{jdata.t2.[1].nat}}<span id="sid1"></span></b></td>
...
if(${user.usertype} ==4){
	...
	userId ='${user.parentid}';
     	...
     	var starCountRefResult = secondaryApp.database().ref(livedt1daydata+"/result");
		starCountRefResult.on('value', (snapshot) =>{
		    const result = snapshot.val();
		    console.log(result)
			
			$("#last-result").html('');
			var source   = $("#dtonedayoddsresulthandler").html();
	    	  var template = Handlebars.compile(source);
...
var marketId =null;
		function loadHtml(){
			$.ajax({
						type:'GET',
						url:'<s:url value="/api/DTOneDayData/"/>',
						success: function(jsondata, textStatus, xhr) {
							if(xhr.status == 200){ 		
								 ...
								 $("#vlucky7div").html('');
								  ...
								  //   console.log(obj)
								     var starCountRef = secondaryApp.database().ref(livedt1daydata+"/data/data");
