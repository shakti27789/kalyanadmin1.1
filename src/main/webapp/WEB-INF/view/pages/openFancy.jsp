             ...
             <table id="myTable" class="table table-striped table-bordered " style="width:100%">
  			...
  			{{#each data}}
                  <tr>   
						...
						<td>{{result}} <input type='hidden' id ='result{{id}}' value='{{result}}'</td>
						<td>{{resultdeclareby}}</td>
							<td><button type="button" class="btn btn-info blue-btn btn-sm" onclick = "rollbackFancy('{{id}}','{{fancyid}}','{{matchid}}')">RollBack</button></td>
    ...
    {{#if_greaterThan data.length 0}}	  
  			...
  			{{#each data}}
                  <tr>
						...
						<td>SUSPENDED<input type='hidden' id ='result{{id}}' value='{{result}}'</td>
					{{#if_eq ${user.usertype} 4}}
						<td><button type="button" class="btn btn-info blue-btn btn-sm" onclick = "rollbackSuspendedFancy('{{fancyid}}','{{eventid}}')">RollBack</button></td>
					{{/if_eq}}	                   
				</tr>
 			{{/each}}          
                </tbody>
...
<!-- ************** end   ******************** -->
    ...
    {
    	$("#fancy").append('<div class="loader"></div>')
		
   		$.ajax({
   			type:'GET',
   			url:'<s:url value="/api/getFancyResult/"/>'+startpage+"/1000",
  			success: function(jsondata, textStatus, xhr) {
    				if(xhr.status == 200)
    				{     
    					$("#noFancyResult").hide();
    ...
    function showMatch(type,status){
		    ...
		    $.ajax({
		    					type:'GET',
		    					url:'<s:url value="api/getMatchBymarketnameforfancy"/>?status='+status,
		    					contentType:"application/json; charset=utf-8",
		    					success: function(jsondata, textStatus, xhr) {
		    ...
		    function getFancy()
		{
		    	...
		    	if($("#matchid").val()!="-1"){
		    		$.ajax({
						type:'GET',
						url:'<s:url value="/api/getFancyByMatch?matchid="/>'+$("#matchid").val(),
						contentType:"application/json; charset=utf-8",
						success: function(jsondata, textStatus, xhr) {
							if(xhr.status == 200)
							{    $(".loader").fadeOut("slow");
		    ...
		    function getSuspendedFancy()
		    {
		   		$.ajax({
		   			type:'GET',
		   			url:'<s:url value="/api/getSuspendedFancy"/>',
		  			success: function(jsondata, textStatus, xhr) {
		    				if(xhr.status == 200)
		    				{     
